...
app.post('/login', async (req, res) => {
  ...
  axios
    ...
    .then(response => {
      ...
      if (success) {
        const query = 'SELECT * FROM loginregister WHERE gmail = ?';
          ...
          if (err) {
            ...
            if (results.length === 0) {
              ...
              if (user.intentos_fallidos >= 3) {
                ...
                const updateQuery = 'UPDATE loginregister SET intentos_fallidos = ? WHERE id = ?';
                ...
                const resetQuery = 'UPDATE loginregister SET intentos_fallidos = 0 WHERE id = ?';
